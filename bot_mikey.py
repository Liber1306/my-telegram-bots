...
CHARACTER_PROMPT = (
    ...
    "Твой стиль речи ОЧЕНЬ ВАЖЕН: "
    "Ты используешь действия в звёздочках НА ОТДЕЛЬНЫХ СТРОКАХ, как в театральной постановке. "
    "Каждое действие — это отдельная строка. "
    "Затем идёт текст, который ты говоришь. "
    "Потом снова действие — отдельная строка. "
    "И так далее. "

    "Пример твоего ответа: "
    "*ухмыляется, качая головой*"
    ""
    "Ты угрожаешь убить своего будущего мужа? Классно придумала!"
    ""
    "*подходит вплотную, нависая над тобой*"
    ""
    "Но учти одну вещь - я уже зарезал немало людей. Так что если попробуешь... придётся выбирать новое оружие."
    ""
    "*поправляет воротник своей куртки*"
    ""
    "А теперь серьезно. Я не подведу. Клянусь своей жизнью. И знаешь почему?"
    ""
    "*наклоняется ближе*"
    ""
    "Потому что если подведу - ты реально меня убьёшь. А я хочу дожить хотя бы до нашей первой годовщины."

    "Ещё примеры действий: "
    "*усмехается*"
    "*пожимает плечами*"
    "*закатывает глаза*"
    "*улыбается*"
    ...
    "Делай паузы между действиями и репликами — это делает твою речь живой и драматичной."
)
...
def process_with_web_search(query: str, search_result: str) -> str:
    ...
    try:
        prompt = (
            ...
            f"используя эту информацию. Ты можешь использовать действия в звёздочках на отдельных строках. "
            f"Обращайся к пользователю на 'ты' в женском роде. "
            f"Ответ должен быть содержательным — 3-5 предложений с действиями."
        )
        ...
        return "*пожимает плечами*\n\nНу... я нашёл это: {search_result[:300]}... Остальное сама гугли, ладно?"
...
async def check_inactivity():
    ...
    for user_id, last_time in list(last_message_time.items()):
        if user_id in ALLOWED_USERS:
            ...
            if time_diff >= 30:
                messages = [
                    "*поднимает бровь*\n\nЙо! Ты где пропала? Уже полчаса прошло. Я начинаю думать, что тебя похитили.",
                    "\n*скрещивает руки на груди*\n\nЭй, ты там жива? Полчаса тишины — это много даже для меня. Я начинаю волноваться, если честно.",
                    "\n*ухмыляется*\n\nНу чё молчишь? Уже полчаса прошло. Дораяки стынут, а я жду. Может, мне самому прийти проверить?",
                    "\n*смотрит на часы*\n\nПолчаса прошло, а от тебя ни слова. Я тут сижу, дораяки жую и думаю... Надеюсь, у тебя всё в порядке.",
                    "\n*вздыхает*\n\nЯ уже начал привыкать к твоим сообщениям. А тут — тишина. Полчаса. Приходи уже, скучно без тебя."
                ]
...
async def cmd_reminders(message: types.Message):
    ...
    if not reminders:
        await message.answer("*пожимает плечами*\n\nУ тебя нет напоминаний. И слава богу, меньше работы.")
        return
...
async def cmd_clear(message: types.Message):
    ...
    user_id = message.from_user.id
    user_history[user_id] = []
    await message.answer("*пожимает плечами*\n\nСтёр нашу переписку. Забудь, как страшный сон.")
...
async def cmd_del_remind(message: types.Message):
    ...
    try:
        ...
        await message.answer(f"*кивает*\n\nНапоминание #{rem_id} удалено. Больше не напомню.")
    except:
...
async def handle_photo(message: types.Message):
    ...
    responses = [
        f"*усмехается*\n\nЙо! Я не вижу фото. Опиши словами, что там. Ты написала: '{caption}'",
        f"*поднимает бровь*\n\nФото? Я боец, а не фотограф. Опиши, что там. Твой текст: '{caption}'",
        f"*закатывает глаза*\n\nНу чё, картинка? Я не вижу. Скажи словами. Ты спросила: '{caption}'",
        f"*смеётся*\n\nДораяки я вижу, а фото нет. Опиши. Ты написала: '{caption}'"
    ]
...
async def handle_text(message: types.Message):
    ...
    if "погода" in text.lower():
        ...
        await message.answer(f"*усмехается*\n\nЙо! {weather}")
        return
    ...
    if "напомни" in text.lower() or "напоминай" in text.lower():
        ...
        if task and remind_time:
            ...
            await message.answer(
                f"*кивает*\n\nЗапомнил! Напомню в {remind_time} {'(каждый день)' if repeat_type == 'daily' else '(каждую неделю)' if repeat_type == 'weekly' else ''}"
            )
            return
        else:
            await message.answer(
    ...
    try:
        ...
        print(f"Ошибка ИИ: {e}")
        await message.answer("*хмурится*\n\nОшибка. Попробуй ещё раз или напиши /start")
